    ...
    fmap[#_FNC_] = math_functor<double, _##_FNC_>;
        ...
        struct _int_truncate {
            int operator()(double x) noexcept
            {
                ...
                return int((x < 0.0F)
                        ? int(
                            (abs(x)) - int(abs(x)) > 0.0F ? int(x) - 1 : int(x))
                        : int(x));
            }
        };
        ...
        */
        ...
        auto functor_errno_str = [](rt_prog_ctx_t& ctx, const std::string& name,
            ...
            return nu::variant_t(strerror(int(vargs[0].to_int())));
        };
        ...
        auto functor_getwindowdy
            = [](rt_prog_ctx_t& ctx, const std::string& name,
                  ...
                  (void)ctx;
                  check_arg_num(args, 0, name);
                  return nu::variant_t(integer_t(_os_get_window_dy()));
              };
        ...
        auto functor_set_topmost
            = [](rt_prog_ctx_t& ctx, const std::string& name,
                  ...
                  (void)ctx;
                  check_arg_num(args, 0, name);
                  return nu::variant_t(integer_t(_os_set_topmost()));
              };
        ...
        // GetDateTime() — returns a DateTime struct with all fields at once.
        // Preferred over the individual Sys* scalar functions (deprecated
        // v2.0).
        auto functor_getdatetime
            = [](rt_prog_ctx_t& ctx, const std::string& name,
                  ...
                  (void)ctx;
                  check_arg_num(args, 0, name);
                  return nu::_os_get_datetime_struct();
              };
