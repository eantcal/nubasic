    ...
    rt_prog_ctx_t& ctx, const std::string& fname, const func_args_t& args)
{
    ...
    if (n_of_p != 3 && n_of_p != 2) {
        throw exception_t(std::string("Syntax error: expected 2 or 3 "
                                      "parameters: hash, key[, value=false]"));
    }
    ...
    if (n_of_p == 2) {
        ctx.hash_tbls[hash_name.to_str()][hash_key.to_str()] = variant_t(false);
    } else {
    ...
    rt_prog_ctx_t& ctx, const std::string& fname, const func_args_t& args)
{
    ...
    if (n_of_p > 2 || n_of_p < 1) {
        throw exception_t(std::string(
            "Syntax error: expected 1 or 2 parameters: hash[, key]"));
    }
    ...
    rt_prog_ctx_t& ctx, const std::string& fname, const func_args_t& args)
{
    ...
    return variant_t(integer_t(tbl.size()));
}
