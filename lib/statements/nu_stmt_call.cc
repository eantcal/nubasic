    ...
    rt_prog_ctx_t& ctx, const prog_pointer_t::line_number_t& line)
{
    ...
    const auto i = ctx.proc_prototypes.data.find(_name);
    ...
    // Build ByRef writeback frame (before entering scope)
    {
        std::vector<rt_prog_ctx_t::byref_entry_t> frame;
        auto parg = function_prototype.parameters.begin();
        auto aarg = _args.begin();
        for (; parg != function_prototype.parameters.end(); ++parg, ++aarg) {
