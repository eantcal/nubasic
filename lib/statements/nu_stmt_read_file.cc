...
void stmt_read_file_t::run(rt_prog_ctx_t& ctx)
{
    auto rt_error_if
    ...
    rt_error_if(
        _args.size() != 1, rt_error_code_t::value_t::E_WRG_NUM_ARGS, "Read#");
    ...
    rt_error_if(const_var, rt_error_code_t::value_t::E_CANNOT_MOD_CONST,
        "Read# " + name);
    ...
    case nu::variable_t::type_t::INTEGER:
        rt_error_if(size != sizeof(integer_t),
    ...
    case nu::variable_t::type_t::DOUBLE:
        rt_error_if(size != sizeof(double_t),
            rt_error_code_t::value_t::E_TYPE_ILLEGAL,
            "Read# (size=" + to_string(size) + ")");
    ...
    if (is_vector) {
        rt_error_if(!scope->is_defined(name),
            rt_error_code_t::value_t::E_VAR_UNDEF, "Read# '" + name + "'");
        ...
        case variable_t::type_t::INTEGER:
            try {
                scope->define(
                    name, var_value_t(integer_t(llvalue), VAR_ACCESS_RW));
            } catch (...) {
        ...
        case nu::variable_t::type_t::OBJECT:
            rt_error_if(
                true, rt_error_code_t::value_t::E_TYPE_ILLEGAL, "Read#");
            break;
        }
    }
