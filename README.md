...
- **nuRCADE engine (Windows)** — integrated raycaster for pseudo-3D, *Wolfenstein 3D*-style first-person scenes, driven from BASIC via the `Ray…` API; enabled by default on Windows builds (`NUBASIC_WITH_RAYCAST`)
...
## nuBASIC 2.0 for Windows: nuRCADE Integration

<p align="center">
  <img width="720" alt="nuRCADE - New (nu) Raycasting Classic Arcade Development Engine" src="docs/images/nurcade-logo.png" />
</p>

In keeping with its retro spirit, nuBASIC 2.0 for Windows integrates **nuRCADE**, a lightweight raycasting engine for creating classic 90s-style first-person shooters directly in BASIC.
...
- **[Raycast Game Engine](https://github.com/eantcal/nubasic/wiki/Raycast-Game-Engine)** — nuRCADE render loop and full `Ray…` API reference (Windows)
