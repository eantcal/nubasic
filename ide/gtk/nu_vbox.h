...
class vbox_t {
public:
    GtkWidget* get_internal_obj() const noexcept { return _vbox; }
    ...
    void pack_start(const Child& child, int padding = 2, bool expand = false,
        bool fill = false) const noexcept
    {
        gtk_box_pack_start(GTK_BOX(_vbox), child.get_internal_obj(),
