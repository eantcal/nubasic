...
class window_t {
public:
    GtkWidget* get_internal_obj() const noexcept { return _window; }

    explicit window_t(GtkWindowType type = GTK_WINDOW_POPUP)
        : // GTK_WINDOW_TOPLEVEL
    ...
    explicit window_t(GtkWidget* window)
        : _window(window)
    {
        assert(window);
    }
    ...
    void maximize() const noexcept { gtk_window_maximize(GTK_WINDOW(_window)); }

    void show() const noexcept { gtk_widget_show(_window); }
    ...
    template <class CallBack> void on_destroy(CallBack cbk)
    {
        signal_connect<CallBack>("destroy", cbk);
    }

    template <class CallBack> void on_delete_event(CallBack cbk)
    {
        signal_connect<CallBack>("delete_event", cbk);
    }

    template <class Widget> void add_container(const Widget& widget)
    {
        gtk_container_add(GTK_CONTAINER(_window), widget.get_internal_obj());
    }
