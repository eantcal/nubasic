...
public:
     ...
     */
    explicit json_obj_t(data_t* value) noexcept
        : _type(type_t::OBJECT)
    {
     ...
     */
    explicit json_obj_t(array_t* value) noexcept
        : _type(type_t::ARRAY)
    {
        if (!value) {
            value = new array_t;
    ...
    json_obj_t(const string_t& value) noexcept
        : _type(type_t::STRING)
    {
    ...
    //! Constructs a json object from a string object
    explicit json_obj_t(string_t* value) noexcept
        : _type(type_t::STRING)
    {
        assert(_value.ptr);
        _value.ptr = value;
    ...
    explicit json_obj_t(const char_t* value) noexcept
        : _type(type_t::STRING)
    {
        if (!value) {
    ...
    explicit json_obj_t(const bool& value) noexcept
    ...
    json_obj_t(const uint64_t& value) noexcept
        : _type(type_t::U64)
    {
        _value.u64 = value;
    }
    ...
    json_obj_t(const int64_t& value) noexcept
        : _type(type_t::I64)
    {
        _value.i64 = value;
    }
    ...
    json_obj_t(const double& value) noexcept
        : _type(type_t::FLOATING)
    {
        _value.d64 = value;
    }
    ...
    const type_t& get_type() const noexcept { return _type; }

    //! Returns true if object represents null, false otherwise
    bool is_null() const noexcept { return get_type() == type_t::NIL; }
    ...
    bool is_unsigned() const noexcept { return get_type() == type_t::U64; }

    //! Returns true if object represents a boolean value, false otherwise
    bool is_bool() const noexcept { return get_type() == type_t::BOOLEAN; }
     ...
     * @return true if conversion is perfomed, false otherwise
     */
    template <class T> bool to_this_type(T& value)
    {
     ...
     */
    template <class T> bool push_back(const T& item)
    {
     ...
     */
    template <class T> static handle_t make(const T& value)
    {
     ...
     * @return a std::unique_ptr to the object instance
     */
    template <class T> static handle_t make(T&& value)
    {
     ...
     * @return true if operation is performed, false otherwise
     */
    template <class T> bool add_member(const string_t& key, const T& item)
    {
     ...
     */
    template <class T> bool convert_to(T& value)
    {
        return _convert_to(value, std::is_floating_point<T>());
    }

private:
    template <class T> bool _convert_to(T& value, std::true_type)
    {
        double dvalue = 0;
    ...
    template <class T> bool _convert_to(T& value, std::false_type)
    {
        uint64_t ivalue = 0;
