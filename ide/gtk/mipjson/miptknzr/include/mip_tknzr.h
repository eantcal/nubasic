...
private:
    ...
    enum class get_t { JUST_TKN, WHOLE_LN };
    ...
    static bool _is_prefix_ch(
        const char_t ch, const std::set<string_t>& tknset);
    ...
    bool _getline(_istream& is, string_t& line, string_t& eol_s, bool& eof);

    std::unique_ptr<token_t> _extract_comment(string_t& comment,
