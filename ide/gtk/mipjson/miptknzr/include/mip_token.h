...
public:
    ...
    token_t(const tcl_t& type, const string_t& value, size_t line,
        size_t column, char_t quote = 0, char_t esc = 0) noexcept
        : _type(type)
        , _value(value)
        , _line(line)
        , _offset(column)
    ...
    //! return token type
    tcl_t type() const noexcept { return _type; }

    //! return token value
    const string_t& value() const noexcept { return _value; }
