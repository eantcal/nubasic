...
public:
    ...
    {
        if (!_window) {
            const char* title = _replace_controls ? "Replace" : "Find";
            ...
            g_signal_connect(
                _window, "response", G_CALLBACK(gtk_widget_destroy), NULL);
            ...
            gtk_box_pack_start(
                GTK_BOX(gtk_dialog_get_content_area(GTK_DIALOG(_window))), vbox,
                TRUE, TRUE, 0);
            ...
            if (_replace_controls) {
                ...
                gtk_entry_set_text(
                    GTK_ENTRY(_replace_entry), _replace_text.c_str());
            ...
            if (_replace_controls) {
                ...
                g_signal_connect(
                    replace_button, "clicked", G_CALLBACK(replace), entry);
                ...
                g_signal_connect(
                    replace_button, "clicked", G_CALLBACK(replace_all), entry);
                gtk_widget_show(replace_button);
                gtk_container_add(GTK_CONTAINER(vbox), replace_button);
            }
            ...
            _match_word_only
                = gtk_check_button_new_with_mnemonic("Match _Word only");
            ...
            gtk_entry_set_icon_from_stock(
                GTK_ENTRY(entry), GTK_ENTRY_ICON_SECONDARY, GTK_STOCK_CLEAR);

            g_signal_connect(
                entry, "icon-press", G_CALLBACK(icon_press_cb), NULL);
            ...
            // Create a second radio button, and add it to the same group as
            // _btn_forward
            _btn_backward = gtk_radio_button_new_with_label_from_widget(
                GTK_RADIO_BUTTON(_btn_forward), "Backward");
            ...
            gtk_toggle_button_set_active(
                GTK_TOGGLE_BUTTON(_btn_backward), FALSE);
        }
    ...
    const std::string& get_text() const noexcept { return _find_text; }
    ...
    void register_observer(observer_t* obj) { _observer = obj; }
...
private:
    ...
    static void find_and_replace(GtkButton* button, GtkEntry* entry, op_t op)
    {
        ...
        auto forward = GTK_TOGGLE_BUTTON(_this._btn_forward)->active;
        ...
        auto match_case = GTK_TOGGLE_BUTTON(_this._match_case)->active;
    ...
    static void clear_entry(GtkEntry* entry) { gtk_entry_set_text(entry, ""); }
        ...
        GtkEntry* entry, GtkMenu* menu, gpointer user_data)
    {
        ...
        g_signal_connect_swapped(
            item, "activate", G_CALLBACK(clear_entry), entry);
...
public:
    void set_text(const char* text) noexcept { _find_text = text; }


    /* ---------------------------------------------------------------------- */

    void set_replace_text(const char* text) noexcept { _replace_text = text; }


    /* ---------------------------------------------------------------------- */

    int get_flags() const noexcept { return _flgs; }


    /* ---------------------------------------------------------------------- */

    bool is_forward() const noexcept { return _search_direction_forward; }
