...
public:
    ...
    GtkWidget* get_internal_obj() const noexcept { return _editor; }
    ...
    template <class Notify> void set_notify_cbk(Notify cbk) const noexcept
    {
    ...
    sptr_t cmd(M m, W w, L l) const noexcept
    {
        return scintilla_send_message(
            SCINTILLA(_editor), m, (sptr_t)w, (sptr_t)l);
    }

    template <class M, class W> sptr_t cmd(M m, W w) const noexcept
    {
        return scintilla_send_message(
            SCINTILLA(_editor), m, (sptr_t)w, (sptr_t)0);
    }

    template <class M> sptr_t cmd(M m) const noexcept
    {
        return scintilla_send_message(
            SCINTILLA(_editor), m, (sptr_t)0, (sptr_t)0);
    }
    ...
    void go_to_pos(long pos) const noexcept { cmd(SCI_GOTOPOS, pos, 0); }
    ...
    void refresh() const noexcept { cmd(SCI_COLOURISE, 0, -1); }
    ...
    {
        return int(cmd(
                   SCI_LINEFROMPOSITION, int(cmd(SCI_GETCURRENTPOS, 0, 0)), 0))
            + 1;
    }

    int get_current_colum() const noexcept
    {
        return int(cmd(SCI_GETCOLUMN, int(cmd(SCI_GETCURRENTPOS, 0, 0)), 0))
            + 1;
    }
    ...
    {
        const auto line = int(cmd(SCI_MARKERNEXT, get_current_line(), 0xffff));
    ...
    void set_item_style(int style, int fore, int back = 0xffffff, int size = 0,
