...
try {
    ...
    Write-Host '>>> load "test_interruptible_input_str.bas"'
    $process.StandardInput.WriteLine('load "test_interruptible_input_str.bas"')
    ...
    if ($output -notmatch 'before input\$') {
        throw "The INPUT$ prelude line was not emitted."
    }
