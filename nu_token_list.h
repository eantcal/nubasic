...
public:
   ...
   friend token_list_t operator+(
      const token_list_t& ltknl,
      const token_list_t& rtknl)
   {
   ...
   //!In case such a list does not exist, returns an iterator
   //!referring to the past-the-end element of the list
   data_t::reverse_iterator skip_left(
   ...
   size_t find(const std::string& identifier, size_t pos = 0, size_t items = 0);
      ...
      bool b_erase = false)
   {
      return sublist(
                [&](const token_t& t)
      {
         return t.identifier() == first;
      },
      [&](const token_t& t)
      {
         return t.identifier() == second;
      },
      search_from, b_erase);
   }
