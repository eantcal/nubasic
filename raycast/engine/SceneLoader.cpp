...
namespace {
	...
	const nlohmann::json* selectLayer(
		...
		std::vector<std::string>& errors)
	{
		...
		if (!selectedLayerId.empty()) {
			errors.push_back("World does not contain layer '" + selectedLayerId + "'.");
			return nullptr;
		}
		...
		const std::string& layerId)
	{
		...
		for (const auto& entry : node["spriteInstances"]) {
			...
			sprite.verticalOffsetCells = readDouble(entry, "verticalOffsetCells", 0.0);
			sprite.collisionRadiusCells = readDouble(entry, "collisionRadiusCells", 0.0);
			...
			sprite.speedCellsPerSecond = readDouble(entry, "speedCellsPerSecond", 0.0);
			sprite.detectionRadiusCells = readDouble(entry, "detectionRadiusCells", 0.0);
			sprite.patrolRadiusCells = readDouble(entry, "patrolRadiusCells", 0.0);
			sprite.engagementHysteresisCells = readDouble(entry, "engagementHysteresisCells", 0.5);
			sprite.patrolCircuit = readBool(entry, "patrolCircuit", false);
			sprite.stoppingDistanceCells = readDouble(entry, "stoppingDistanceCells", 0.0);
			...
			sprite.attackCooldownSeconds = readDouble(entry, "attackCooldownSeconds", 1.0);
			...
			sprite.attackBurstPauseSeconds = readDouble(entry, "attackBurstPauseSeconds", 1.2);
			sprite.pickupHealth = readDouble(entry, "pickupHealth", 0.0);
			sprite.unlocksMap = readBool(entry, "unlocksMap", false);
			sprite.savePoint = readBool(entry, "savePoint", false);
			...
			sprite.explosiveHitPoints = readDouble(entry, "explosiveHitPoints", 45.0);
			sprite.explosionRadiusCells = readDouble(entry, "explosionRadiusCells", 0.0);
			sprite.explosionDamage = readDouble(entry, "explosionDamage", 0.0);
			sprite.explosionScaleCells = readDouble(entry, "explosionScaleCells", 1.5);
			sprite.explosionSpriteSet = readString(entry, "explosionSpriteSet", std::string());
			sprite.destroyedSpriteSet = readString(entry, "destroyedSpriteSet", std::string());
			sprite.destroyedScaleCells = readDouble(entry, "destroyedScaleCells", 0.55);
			if (entry.contains("damageResponse")) {
				const auto& response = entry["damageResponse"];
				if (!response.is_object()) {
					errors.push_back("spriteInstances damageResponse must be an object.");
				}
				else {
					sprite.damageResponseType =
						readString(response, "type", std::string());
					sprite.damageResponseHitPoints =
						readDouble(response, "hitPoints", sprite.explosiveHitPoints);
					sprite.damageResponseEffectSpriteSet =
						readString(response, "effectSpriteSet",
					...
					sprite.damageResponseDamage =
						readDouble(response, "damage",
	...
	SceneLoader::BackgroundMusic readBackgroundMusic(
		const nlohmann::json& node,
		...
		std::vector<std::string>& errors)
	{
		...
		music.volumePercent = clampPercent(
			readDouble(value, "volumePercent", readDouble(value, "volume", music.volumePercent)));
		return music;
	}

	bool readPlayerWeapon(
	...
	const std::string& layerId) const
{
	...
	std::string activeLayerId;
	const auto* layer = selectLayer(document, layerId, activeLayerId, result.errors);
	if (layer != nullptr) {
		result.scene.activeLayerId = activeLayerId;
	}

	result.scene.brightness = std::clamp(
		readDouble(document, "brightness", 1.0),
		0.05,
		2.0);
	if (layer != nullptr) {
		result.scene.brightness = std::clamp(
			readDouble(*layer, "brightness", result.scene.brightness),
			0.05,
			2.0);
	}
	result.scene.depthShading = std::clamp(
		readDouble(document, "depthShading", 100.0),
		1.0,
		1000.0);
	if (layer != nullptr) {
		result.scene.depthShading = std::clamp(
			readDouble(*layer, "depthShading", result.scene.depthShading),
			1.0,
			1000.0);
	}
	...
	if (result.scene.playerWeapons.empty()) {
		...
		const auto existingActive = std::find_if(
			result.scene.playerWeapons.begin(),
	...
	result.scene.backgroundMusic =
		readBackgroundMusic(document, result.scene.backgroundMusic, result.errors);
	if (layer != nullptr) {
		result.scene.backgroundMusic =
			readBackgroundMusic(*layer, result.scene.backgroundMusic, result.errors);
	}

	const auto* playerStartNode = &document;
	if (layer != nullptr && layer->contains("playerStart") && (*layer)["playerStart"].is_object()) {
	...
	if (playerStartNode->contains("playerStart") && (*playerStartNode)["playerStart"].is_object()) {
		...
		result.scene.playerStart.facingDegrees = readDouble(playerStart, "facingDegrees", 0.0);
		result.scene.hasPlayerStart = true;
	}

	if (document.contains("playerStats") && document["playerStats"].is_object()) {
		const auto& playerStats = document["playerStats"];
		result.scene.playerStats.maxHealth =
			readDouble(playerStats, "maxHealth", result.scene.playerStats.maxHealth);
		result.scene.playerStats.health =
			readDouble(playerStats, "health", result.scene.playerStats.maxHealth);
	}
	...
	readSpriteInstances(document, result.scene.spriteInstances, result.errors, std::string());
	if (layer != nullptr) {
		readSpriteInstances(
			*layer,
			result.scene.spriteInstances,
			result.errors,
