...
namespace {
		...
		if (node.contains("faceTextures") && node["faceTextures"].is_object()) {
			for (const auto& faceEntry : node["faceTextures"].items()) {
				...
				if (tryParseWallFace(faceEntry.key(), face)
					...
					&& tryParseHexByte(faceEntry.value().get<std::string>(), faceTextureKey)) {
					span.faceTextureKeys[static_cast<size_t>(face)] = faceTextureKey;
				}
			}
		}
		...
		if (node.contains("facesEnabled") && node["facesEnabled"].is_object()) {
			for (const auto& faceEntry : node["facesEnabled"].items()) {
				...
				if (tryParseWallFace(faceEntry.key(), face)
					...
					span.facesEnabled[static_cast<size_t>(face)] = faceEntry.value().get<bool>();
				}
			}
		}

		const auto interiorTextureText = readString(node, "interiorTexture", std::string());
	...
	DoorDefinition buildDoor(
		const nlohmann::json& blockNode,
		...
		WorldJsonLoader::Result& result)
	{
		...
		if (!blockNode["door"].is_object()) {
			result.errors.push_back("Block '" + blockKey + "' has an invalid door object.");
			return door;
		}
		...
		door.requiredKey = readString(
			doorNode,
			"requiredKey",
			readString(doorNode, "key", door.requiredKey));
		door.triggerDistanceCells = readDouble(
			doorNode,
			"triggerDistanceCells",
			door.triggerDistanceCells);
		door.openTimeSeconds = readDouble(
			doorNode,
			"openTimeSeconds",
			door.openTimeSeconds);
		door.closeDelaySeconds = readDouble(
			doorNode,
			"closeDelaySeconds",
			door.closeDelaySeconds);
		door.openSound = readString(
			doorNode,
			"openSound",
			readString(doorNode, "openingSound", door.openSound));
		door.openSoundVolumePercent = readInt(
			doorNode,
			"openSoundVolumePercent",
			door.openSoundVolumePercent);
			...
			for (const auto& frameNode : doorNode["frames"]) {
				if (!frameNode.is_string()) {
					result.errors.push_back(
						"Block '" + blockKey + "' has a door frame that is not a texture id.");
					continue;
				}
				...
				if (!tryParseHexByte(frameText, frameKey)) {
					result.errors.push_back(
						"Block '" + blockKey + "' has an invalid door frame texture id '" +
						frameText + "'.");
					continue;
				}
		...
		const auto readLockedOverlays = [&](const char* propertyName) {
			...
			if (!doorNode[propertyName].is_object()) {
				result.errors.push_back(
					"Block '" + blockKey + "' has an invalid door " +
					std::string(propertyName) + " object.");
				return;
			}
			...
			for (const auto& overlayEntry : doorNode[propertyName].items()) {
				...
				if (!overlayEntry.value().is_string()) {
					result.errors.push_back(
						"Block '" + blockKey + "' has a door overlay that is not a texture id.");
					continue;
				}
				...
				if (!tryParseHexByte(overlayText, overlayKey)) {
					result.errors.push_back(
						"Block '" + blockKey + "' has an invalid door overlay texture id '" +
						overlayText + "'.");
					continue;
				}
		...
		if (door.openTimeSeconds <= 0.0) {
			result.errors.push_back(
				"Block '" + blockKey + "' has a door openTimeSeconds value that must be positive.");
			door.openTimeSeconds = 0.45;
		}

		if (door.triggerDistanceCells < 0.0) {
			result.errors.push_back(
				"Block '" + blockKey + "' has a negative door triggerDistanceCells value.");
			door.triggerDistanceCells = 0.0;
		}

		if (door.closeDelaySeconds < 0.0) {
			result.errors.push_back(
				"Block '" + blockKey + "' has a negative door closeDelaySeconds value.");
		...
		WorldJsonLoader::Result& result)
	{
		...
		if (!blockNode["animations"].is_array()) {
			result.errors.push_back("Block '" + blockKey + "' has an invalid animations array.");
			return;
		}

		for (const auto& animationNode : blockNode["animations"]) {
			if (!animationNode.is_object()) {
				result.errors.push_back("Block '" + blockKey + "' has an animation that is not an object.");
				continue;
			}
			...
			const auto isWallOverlay = target == "walloverlay" || target == "overlay";
			if (!isWallBase && !isWallOverlay) {
				continue;
			}

			const auto wallIndex = readInt(animationNode, "wallIndex", -1);
			if (wallIndex < 0 || wallIndex >= static_cast<int>(block.walls.size())) {
				result.errors.push_back(
					"Block '" + blockKey + "' has an animation with an invalid wall index.");
				continue;
			}

			int face = -1;
			if (!tryParseAnimationFace(readString(animationNode, "face", "all"), face)) {
				result.errors.push_back(
					"Block '" + blockKey + "' has an animation with an invalid face.");
				continue;
			}
			...
			if (!animationNode.contains("frames") || !animationNode["frames"].is_array()) {
				result.errors.push_back(
					"Block '" + blockKey + "' has a wall animation without frames.");
				continue;
			}
			...
			for (const auto& frameNode : animationNode["frames"]) {
				if (!frameNode.is_string()) {
					result.errors.push_back(
						"Block '" + blockKey + "' has an animation frame that is not a texture id.");
					continue;
				}
				...
				if (!tryParseHexByte(frameText, frameKey)) {
					result.errors.push_back(
						"Block '" + blockKey + "' has an invalid animation frame texture id '" +
						frameText + "'.");
					continue;
				}
			...
			if (animation.textureKeys.empty()) {
				result.errors.push_back(
					"Block '" + blockKey + "' has a wall animation with no valid frames.");
				continue;
			}
	...
	const nlohmann::json* selectWorldLayer(
		const nlohmann::json& document,
		const std::string& requestedLayerId,
		...
		WorldJsonLoader::Result& result)
	{
		...
		if (!selectedLayerId.empty()) {
			result.errors.push_back("World JSON does not contain layer '" + selectedLayerId + "'.");
			return nullptr;
		}
	...
	const std::string& layerId) const
{
	...
	try {
		...
		result.errors.push_back(std::string("Invalid world JSON: ") + error.what());
		return result;
	}
	...
	if (version != 2) {
		result.errors.push_back("Unsupported world version: " + std::to_string(version));
		return result;
	}

	const auto* layer = selectWorldLayer(document, layerId, result.activeLayerId, result);
	if (layer == nullptr) {
		return result;
	}
	...
	const auto defaultWallHeight = readInt(grid, "defaultWallHeight", cellDepth);
	...
	if (worldNode.contains("playerStart") && worldNode["playerStart"].is_object()) {
		playerStartNode = &worldNode;
	}

	if (playerStartNode->contains("playerStart") && (*playerStartNode)["playerStart"].is_object()) {
		const auto& playerStart = (*playerStartNode)["playerStart"];
		target.setPlayerStartCell(
			readDouble(playerStart, "xCell", 1.5),
			readDouble(playerStart, "yCell", 1.5),
			readDouble(playerStart, "facingDegrees", 0.0));
	}

	const auto* playerTurnNode = &document;
	if (worldNode.contains("playerTurn") && worldNode["playerTurn"].is_object()) {
		playerTurnNode = &worldNode;
	}

	if (playerTurnNode->contains("playerTurn") && (*playerTurnNode)["playerTurn"].is_object()) {
		const auto& playerTurn = (*playerTurnNode)["playerTurn"];
		// Non-positive (or missing) values leave the WorldMap defaults in place.
	...
	if (worldNode.contains("messageLog") && worldNode["messageLog"].is_object()) {
		messageLogNode = &worldNode;
	}

	if (messageLogNode->contains("messageLog") && (*messageLogNode)["messageLog"].is_object()) {
		...
		log.maxLines = clampInt(readInt(messageLog, "maxLines", log.maxLines), 1, 4);
		...
		log.healthPickup = readString(messageLog, "healthPickup", log.healthPickup);
		log.mapUnlocked = readString(messageLog, "mapUnlocked", log.mapUnlocked);
		log.mapActorsUnlocked = readString(messageLog, "mapActorsUnlocked", log.mapActorsUnlocked);
		log.weaponReload = readString(messageLog, "weaponReload", log.weaponReload);
	...
	for (const auto& entry : document["blocks"].items()) {
		...
		if (!entry.value().is_object()) {
			result.errors.push_back("Block '" + entry.key() + "' must be an object.");
			continue;
		}

		BlockDefinition block;
		block.name = readString(entry.value(), "name", std::string());
		block.horizonImage = readString(entry.value(), "horizonImage", std::string());

		const auto legacyPackedCell = readString(entry.value(), "legacyPackedCell", std::string());
		if (!legacyPackedCell.empty()) {
			Cell packedCell = 0;
			if (!tryParsePackedCell(legacyPackedCell, packedCell)) {
				result.errors.push_back(
					"Block '" + entry.key() + "' has an invalid legacyPackedCell.");
				continue;
			}
		...
		if (entry.value().contains("walls") && entry.value()["walls"].is_array()) {
			for (const auto& wallNode : entry.value()["walls"]) {
				...
				if (span.top <= span.bottom) {
					result.errors.push_back(
						"Block '" + entry.key() + "' has an invalid wall span.");
					continue;
				}
		...
		if (block.door.enabled
			...
			&& !block.walls.empty()) {
			block.door.animationTextureKeys.push_back(block.walls.front().textureKey);
		}
	...
	packedCells.reserve(static_cast<size_t>(rows) * static_cast<size_t>(columns));
	std::vector<std::vector<BlockId>> blockIds(rows, std::vector<BlockId>(columns, 0));
	...
	for (int row = 0; row < rows; ++row) {
		...
		for (int column = 0; column < columns; ++column) {
			...
			packedCells.push_back(hasPackedCellOverride[blockId]
				...
				: packCellFromBlock(parsedBlocks[blockId], defaultWallHeight));
		}
	}
	...
	for (int blockId = 0; blockId < 256; ++blockId) {
		if (parsedPresent[blockId]) {
			target.setBlockDefinition(
				static_cast<BlockId>(blockId), std::move(parsedBlocks[blockId]));
		}
	}
