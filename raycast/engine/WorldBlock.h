...
namespace nu::rcade {
	...
	enum class WallSpanKind : uint8_t {
		Solid = 0,
		Transparent = 1
	};

	enum class WallFace : uint8_t {
		North = 0,
		East = 1,
		South = 2,
		West = 3
	...
	struct WallSpan {
		...
		WallSpan(
			MapCell::TextureKey texture,
			int32_t spanBottom,
			int32_t spanTop,
			WallSpanKind spanKind,
			bool collides) noexcept
			: textureKey(texture)
			, bottom(spanBottom)
			, top(spanTop)
			, kind(spanKind)
		...
		MapCell::TextureKey textureForFace(WallFace face) const noexcept
		{
			...
			const auto faceTexture = index < faceTextureKeys.size()
				? faceTextureKeys[index]
				: 0;
