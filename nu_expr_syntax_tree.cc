...
token_list_t expr_syntax_tree_t::remove_blank(token_list_t tl)
{
   ...
   while (i != tl.end())
   {
      if (!is_blank(i->type()))
      {
         if (i->type() == old_token_cl &&
               i->identifier() == old_token_id)
         {
            if (i->type() != tkncl_t::SUBEXP_BEGIN &&
                  i->type() != tkncl_t::SUBEXP_END)
            {
               syntax_error(
                  i->expression(), i->position());
            }
         }
         ...
         if ( i->type() == tkncl_t::SUBEXP_END && old_token_cl == tkncl_t::OPERATOR )
   ...
   token_list_t & tl, token_list_t::data_t::iterator i)
{
   ...
   token_list_t::tkp_t begin = { explicit_left_marker, tkncl_t::SUBEXP_BEGIN }; // (
   token_list_t::tkp_t end = { explicit_right_marker, tkncl_t::SUBEXP_END };    // )
   ...
   subexp.data().push_front(token_t(implicit_left_marker, tkncl_t::SUBEXP_BEGIN, pos, ptr));
   subexp.data().push_back(token_t(implicit_right_marker, tkncl_t::SUBEXP_END, pos, ptr));
   ...
   token_list_t & tl, token_list_t::data_t::iterator i)
{
   ...
   if (i == tl.end() || tl.size()<3 ||
         (i - 1) == tl.end() || (i + 1) == tl.end())
   {
   ...
   if (ops.find(i->identifier()) != ops.end())
   {
      ...
      if (right_sub)
      {
         ...
         right_list =
            tl.sublist(tl.iterator_to_pos(i) + 1, right_size);
      ...
      token_list_t ret_tl = tl.replace_sublist(
                               tl.iterator_to_pos(i - left_size),
                               tl.iterator_to_pos(i + right_size),
                               subexp);
...
void expr_syntax_tree_t::rework_minus_operator(
   token_list_t & tl,
   token_list_t::data_t::iterator i)
{
   ...
   if (ops.find(i->identifier()) != ops.end())
   {
      ...
      subexp.data().push_front(
         token_t("0", tkncl_t::INTEGRAL, i->position(), i->expression_ptr()));
   ...
   token_list_t & tl, token_list_t::data_t::iterator& i)
{
   while (i != tl.end())
   {
            ...
            i->identifier() == implicit_left_marker)
      {
         ...
         token_list_t subexp =
            tl.sublist(begin, end, tl.iterator_to_pos(i));
