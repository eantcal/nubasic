...
| `NUBASIC_WITH_RAYCAST` | `ON` (Windows), `OFF` (other) | all | Build the integrated nuRCADE pseudo-3D engine and the `Ray…` API. See [Raycast Game Engine](Raycast-Game-Engine). |
...
> `cmake --preset raycast-release` / `raycast-debug` to build with nuRCADE enabled
